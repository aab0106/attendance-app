...
const fmtDate = (ts:any, ds?:string) => {
  if(ds) return new Date(ds+"T00:00:00").toLocaleDateString([],{month:"short",day:"numeric"});
  if(!ts) return "—";
  const d = ts.toDate?ts.toDate():new Date(ts);
  return d.toLocaleDateString([],{month:"short",day:"numeric",year:"numeric"});
};
const statusLabel = (v:boolean|null|undefined) =>
  v===true?"Excused":v===false?"Unexcused":"Pending";
const statusColor = (v:boolean|null|undefined) =>
  v===true?"bg-green-100 text-green-700":v===false?"bg-red-100 text-red-700":"bg-amber-100 text-amber-700";
      ...
      <button onClick={()=>setOpen(v=>!v)}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            {records.length} late occurrence{records.length!==1?"s":""} ·
            avg {Math.round(totalMins/records.length)} min late
        ...
        <div className="border-t border-gray-100">
          <table className="w-full">
            <thead><tr className="bg-gray-50">
              {["Date","Punch In","Late By","Status","Reason","Reviewed By"].map(h=>(
              ...
              {records.map((r,i)=>(
                <tr key={r.id} className={`border-t border-gray-50 ${i%2===0?"bg-white":"bg-gray-50/50"}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${statusColor(r.lateApproved)}`}>
                      {statusLabel(r.lateApproved)}
                    </span>
                  </td>
...
export default function LateReportPage() {
  ...
  useEffect(()=>{
    getDocs(collection(db,"users")).then(snap=>{
      const m=new Map(); snap.docs.forEach(d=>m.set(d.id,d.data())); setUserMap(m);
    });
  },[]);
  ...
  const loadData = async()=>{
    ...
    try{
      ...
      const snap=await getDocs(query(collection(db,"attendance"),
        where("lateStatus","==","late"),where("timestamp",">=",start),where("timestamp","<",end)));
  ...
  const enriched = data.map(r=>({
    ...r,
    userName: userMap.get(r.userId)?.name ?? r.userName,
    department: r.department ?? userMap.get(r.userId)?.department,
  }));

  // Filter by tab — IMPORTANT: rejected by manager = lateApproved===false, not pending
  ...
  for(const r of tabFiltered){
    ...
    if(!seen.has(r.userId)){
      const g={userId:r.userId,userName:r.userName,records:[]};
      seen.set(r.userId,g); groups.push(g);
    }
    ...
    <div className="p-8">
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Grouped by employee — click to expand</p>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6 flex items-end gap-4 flex-wrap">
        ...
        <div className="flex-1 min-w-40">
          <label className="block text-sm font-semibold text-gray-600 mb-1.5">Search employee</label>
      ...
      <div className="grid grid-cols-4 gap-3 mb-5">
        {[
          {l:"Total Late",   v:enriched.length,c:"bg-orange-100 text-orange-700"},
          {l:"Pending",      v:pending,         c:"bg-amber-100 text-amber-700"},
          {l:"Excused",      v:excused,          c:"bg-green-100 text-green-700"},
          {l:"Unexcused",    v:unexcused,        c:"bg-red-100 text-red-700"},
        ].map(s=>(
        ...
        {(["all","pending","excused","unexcused"] as const).map(f=>(
          <button key={f} onClick={()=>setFilter(f)}
            className={`px-4 py-2 rounded-xl text-sm font-semibold capitalize transition-colors ${
              filter===f
                ? f==="pending"?"bg-amber-500 text-white":f==="excused"?"bg-green-600 text-white":f==="unexcused"?"bg-red-600 text-white":"bg-blue-600 text-white"
                : "bg-white border border-gray-200 text-gray-600 hover:bg-gray-50"
            }`}>
            {f==="all"?`All (${enriched.length})`:f==="pending"?`Pending (${pending})`:f==="excused"?`Excused (${excused})`:`Unexcused (${unexcused})`}
          </button>
        ))}
        <p className="ml-auto text-sm text-gray-400 self-center">{groups.length} employee{groups.length!==1?"s":""} · click to expand</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-400">
          ...
          <p className="text-sm">{filter==="pending"?"No pending late records — all reviewed!":"No late records found."}</p>
        </div>
      ) : (
